...
class FootballControllerTest < ActionController::TestCase
	...
	test "It should return the welcome message when a user sends the start keyword the first time" do				
		...
		expected = { response: [{ type: "Question", text: @prompt.text, phone_number: @phone_number }, { type: "Response", text: "#{@opt_about.key}. #{@opt_about.text}\r\n#{@opt_my_team.key}. #{@opt_my_team.text}", phone_number: @phone_number }]}
